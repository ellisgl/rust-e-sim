...
pub struct CompiledNetlist {
    ...
    /// Per-relay flat [coil+, coil-, common, NC, NO] compact indices.
    /// Cached so the dynamic relay-stamping path doesn't have to re-resolve
    /// node IDs every Newton iteration.
    pub relay_node_indices: Vec<i32>,
...
pub fn compile_netlist(netlist: &Netlist) -> Option<CompiledNetlist> {
    ...
    for (i, el) in netlist.elements.iter().enumerate() {
        match el {
            ...
            Element::Relay { .. } => relay_indices.push(i),
        }
    }
    ...
    let relay_count = relay_indices.len();
    ...
    // Relay cache: layout [coil+, coil-, common, NC, NO] per relay.
    let mut relay_node_indices: Vec<i32> = Vec::with_capacity(relay_count * 5);
    for &ri in &relay_indices {
        if let Element::Relay {
            coil_positive, coil_negative, common, normally_closed, normally_open, ..
        } = &netlist.elements[ri] {
            relay_node_indices.push(ni(*coil_positive));
            relay_node_indices.push(ni(*coil_negative));
            relay_node_indices.push(ni(*common));
            relay_node_indices.push(ni(*normally_closed));
            relay_node_indices.push(ni(*normally_open));
        }
    }
    ...
    // Relay stamp positions.  Three two-node resistors per relay (coil,
    // common↔NC, common↔NO) — all stamp into the sparsity pattern even
    // if the relay is currently inactive (one contact is Roff = ~1MΩ),
    // because the relay can flip mid-run without recompile.
    for ri in 0..relay_count {
        ...
        let mark_pair = |m: &mut [u8], a: i32, b: i32| {
            if a >= 0 { m[(a as usize) * size + (a as usize)] = 1; }
            if b >= 0 { m[(b as usize) * size + (b as usize)] = 1; }
            if a >= 0 && b >= 0 {
                m[(a as usize) * size + (b as usize)] = 1;
                m[(b as usize) * size + (a as usize)] = 1;
            }
        };
        mark_pair(&mut marker, coil_p, coil_n);
        mark_pair(&mut marker, cmn,    nc);
        mark_pair(&mut marker, cmn,    no);
    }
