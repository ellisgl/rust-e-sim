...
| **BJT junction caps**     | `transient.ts`        | `sim-core/src/transient.rs`| **✓ Phase 4 (with pnjlim)** |
| Dense Gaussian fallback   | `linear.ts`           | `sim-core/src/linear.rs`   | ✓ |
| `Simulator` JS class      | —                     | `sim-wasm/src/lib.rs`      | ✓ |
| Worker integration        | `sim-worker.ts`       | via `solver-engine.ts`     | ✓ Phase 3d (opt-in) |
| **AudioWorklet hosting**  | `speaker-worklet.js`  | `static/audio/sim-rust-worklet.js` | **✓ Phase 4 (opt-in)** |
| Relay state machine       | `transient.ts`        | —                          | Phase 4-cont |
| Source-current diagnostics| `transient.ts`        | —                          | Phase 4-cont |
| Adaptive dt, LTE          | `transient.ts`        | —                          | Phase 4-cont |

### Phase 4 additions

**BJT junction capacitance stamps in transient.**  Phase 3 stored
`tj_cap_volts` but never used them.  Phase 4 stamps two capacitor
companions per BJT each Newton iteration:

```
C_BE = cje + tf · gm     stamped between base & emitter
C_BC = cjc + tr · gmu_b  stamped between base & collector
g    = C / dt            ieq = g · V_prev   (BE only, matches TS)
```

These dominate switching speed in the kit's blocking-oscillator circuits
(metronome, siren).  Without them, BJTs switch instantly and oscillation
runs ~10× too fast at ~20 dB too quiet.  With them — and with pnjlim
re-enabled in the transient path (`Some(&state.node_volts)` rather than
`None`) — audio output is qualitatively similar to the TS engine.

**Audio worklet hosting the Rust simulator.**  A new static-asset worklet
at `static/audio/sim-rust-worklet.js` runs the simulator inside the audio
thread, generating samples sample-by-sample in `process()`.  Compared to
the Phase 3 worker→main→speaker-worklet pipeline (where the simulator
produced audio batches every ~4 ms and a ring-buffer worklet played them
back), Phase 4 eliminates the buffer entirely.  Audio latency drops to
one quantum (~2.7 ms at 48 kHz).

The post-processing chain (DC blocker → tanh compression → 250 Hz Q=0.7
cone bandpass → fade-in) is preserved verbatim from `speaker-worklet.js`
so the audio character is unchanged.

**Build pipeline.**  `rust/build.sh` now copies the wasm-bindgen output
into `static/audio/` alongside the new worklet.  AudioWorkletGlobalScope
has no `fetch`, so the main thread compiles the .wasm and ships the
`WebAssembly.Module` to the worklet via `port.postMessage` (Modules are
structured-cloneable).  The worklet imports the wasm-bindgen JS glue
from a relative path that resolves to its own directory at runtime.

### Phase 4 usage

The TS engine path (worker + ring-buffer worklet) stays the default and
is unchanged.  Opt into Phase 4 by setting `solverEngine = 'rust'` and
routing audio through the new worklet host instead of the existing
speaker worklet.  See `PHASE4-INTEGRATION.md` at the repo root for the
specific Board.svelte edits.

```bash
cd rust && ./build.sh       # builds WASM, copies to static/audio/
ls static/audio/             # sim-rust-worklet.js + sim_wasm.js + sim_wasm_bg.wasm
cd .. && npx svelte-check    # 0 errors
npm run dev
```

### Phase 4 limitations

1. `updateControls` still rebuilds the Rust simulator (audio click on pot
   sweeps).  Same as Phase 3d.  Hot-recompile preserving transient state
   is doable as Phase 4-cont if the glitches matter in practice.
2. Real-time soft-guarantee only: if a sim step takes longer than ~2.7 ms
   wall time, samples drop.  Adaptive-dt heuristic keeps step count bounded
   but doesn't formally guarantee real-time.
3. Relays still skipped — projects using RL1 must use the TS engine.
4. No source-current diagnostics — audio reads node voltages across the
   speaker resistor, not voltage-source branch currents.
